...
import 'package:itms_mobile/presentation/pages/outlets/box-scanning/box_scan_detail_page.dart';
...
class _BoxScanPageState extends State<BoxScanPage> {
  ...
  void initState() {
    ...
    // 从 provider 获取用户信息
    userInfo = _verifyTokenProvider.getUserData();
    _qryLineByEscortNo();
  }

  // 获取线路组织机构列表
  Future<void> _qryLineByEscortNo() async {
    final String? username = userInfo?['username'] as String?;
    if (username == null) return;
    final dynamic lineOrgList = await _service.qryLineByEscortNo(username);
    setState(() {
      lines = (lineOrgList['retList'] as List<dynamic>)
  ...
  Map<String, List<Map<String, dynamic>>> _getGroupedPoints() {
    ...
    final Map<String, List<Map<String, dynamic>>> groupedPoints = {
      '出库网点': (selectedRoute!['deliverOrgNo'] as List).cast<Map<String, dynamic>>(),
      '入库网点': (selectedRoute!['receiveOrgNo'] as List).cast<Map<String, dynamic>>(),
    };
